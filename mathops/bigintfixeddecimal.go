...
// EqualValue
//
// Receives an instances of BigIntFixedDecimal (bigIFxDecComparison)
// and proceeds to compare the numeric value with that of the current
// BigIntFixedDecimal instance (bigIFd).
//
// If the two values are equal, this method returns 'true'.
//
// If the two values are not equal, this method returns 'false'.
//
// If the two 'precision' values are not equal, this method returns
// 'false'.
//
...
// will return true.
func (bigIFd *BigIntFixedDecimal) EqualValue(
	bigIFxDecComparison BigIntFixedDecimal) (bool, error) {
	...
	return new(bigIntFixedDecMolecule).areBigIntFixDecsEqualValue(
		bigIFd, true, &bigIFxDecComparison,
		true, ePrefix)
}
...
func (bigIFd *BigIntFixedDecimal) MultiplyByTenToPower(exponent uint) error {
	...
	ePrefix,
		err = ePref.ErrPrefixDto{}.NewIEmpty(
		nil,
		"BigIntFixedDecimal.MultiplyByTenToPower",
		"")
...
//	BigIntFixedDecimal instance.
//
//	The input parameter 'numSeps' contains numeric
...
func (bigIFd *BigIntFixedDecimal) NewNumStrWithNumSeps(
	numStr string,
	numSeps NumericSeparatorDto) (BigIntFixedDecimal, error) {

	ePrefix := "BigIntFixedDecimal.NewNumStrWithNumSeps() "
	...
	if err != nil {
		...
		return BigIntFixedDecimal{},
			&FuncReturnError{
				...
				ErrContext: fmt.Sprintf("numStr= '%v'\nnumSeps= '%v'\n", numStr, numSeps.String()),
				ErrMessage: err.Error(),
			}
	}

	err = fixedDecimal.SetNumStr(numStr, numSeps.DecimalSeparator)
	...
	if err != nil {
		...
		return BigIntFixedDecimal{},
			&FuncReturnError{
				ErrPrefix:  ePrefix,
				ReturnFunc: "err = fixedDecimal.SetNumStr(numStr, numSeps.DecimalSeparator)",
				ErrContext: fmt.Sprintf("numStr= '%v'\nnumSeps= '%v'\n", numStr, numSeps.String()),
				ErrMessage: err.Error(),
			}
	}
