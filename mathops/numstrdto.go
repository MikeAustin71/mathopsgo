...
type NumStrDto struct {
	...
	absAllNumRunes     	[]rune		// An array of runes containing all the numeric digits in a number with
																...
																//		[]rune{'1','2','3','4','5','6'}
	absIntRunes        	[]rune		// An array of runes containing all the integer elements to the left of
																//		the decimal point. Example: 123.456 = []rune{'1', '2', '3'}
	absFracRunes       	[]rune		// An array of runes containing all the fractional digits to the right.
																//		of the decimal point. Example 123.456 = []rune{'4','5','6'}
...
func (nDto *NumStrDto) IsNumStrDtoValid(errName string) error {
	...
	if lenAbsAllNumRunes != lenAbsIntRunes+lenAbsFracRunes {
		...
		s2 := string(nDto.absIntRunes)
		s3 := string(nDto.absFracRunes)
	...
	checkNumStrOut += string(nDto.absIntRunes)

	if nDto.precision > 0 {
		checkNumStrOut += string(nDto.decimalSeparator)
		checkNumStrOut += string(nDto.absFracRunes)
	}
	...
	for i := 0; i < lenAbsAllNumRunes; i++ {
		...
		if i < lenAbsIntRunes &&
			(nDto.absIntRunes[i] < '0' || nDto.absIntRunes[i] > '9') {
			hasNonNumericChars = true
			break
		}

		if i >= lenAbsIntRunes &&
			(nDto.absFracRunes[i - lenAbsIntRunes] < '0' || nDto.absFracRunes[i - lenAbsIntRunes] > '9') {
			hasNonNumericChars = true
			break
		}
	}
