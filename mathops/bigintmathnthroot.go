...
func (nthrt *BigIntMathNthRoot) Empty() {
	nthrt.NthRoot = new(BigIntNum).NewZero(0)
	nthrt.OriginalRadicand = new(BigIntNum).NewZero(0)
	nthrt.IntBundleRadicand = new(BigIntNum).NewZero(0)
	nthrt.FracBundleRadicand = new(BigIntNum).NewZero(0)
...
func (nthrt *BigIntMathNthRoot) calcPositiveFractionalNthRoot(radicand, nthRoot BigIntNum,
	...
	newNthRoot := new(BigIntNum).NewBigInt(ratFraction.Num(), 0)
	...
	exponent := new(BigIntNum).NewBigInt(ratFraction.Denom(), 0)

	radicandPrecision := new(BigIntNum).NewBigInt(big.NewInt(int64(radicand.GetPrecisionUint())), 0)
