...
func (dec *Decimal) GetIntAry() (IntAry, error) {
	...
	if err != nil {
		...
		fmt.Errorf("Decimal.GetIntAry() - Received error from " +
			"IntAry{}.NewBigInt(dec.signedAllDigitsBigInt," +
			"dec.precision). Error= %v", err)
	}
...
func (dec *Decimal) IsDecimalValid() error {
	...
	signedBigInt, err := dec.numStrDto.GetSignedBigInt()

	if err != nil {
		return fmt.Errorf(ePrefix + "dec.numStrDto.GetSignedBigInt() returned an Error. " +
			"Error='%v' ", err.Error())
	}
	...
	if err != nil {
		return fmt.Errorf(ePrefix + "dec.numStrDto.GetScaleFactor() returned an Error. " +
			"Error='%v' ", err.Error())
	}
...
func (dec *Decimal) MakeDecimalBigIntPrecision(iBig *big.Int, precision uint) (Decimal, error) {
	...
	if err != nil {
		return Decimal{},
		fmt.Errorf(ePrefix + "Error returned by NewNumStr(rQuotient.FloatString(int(d2.precision))) " +
			"Error='%v'", err.Error())
	}
...
func (dec Decimal) NewNumStrDto(numDto NumStrDto) (Decimal, error) {
	...
	newNumStr := numDto.GetNumStr()

	if newNumStr == "" {
		return Decimal{},
		errors.New(ePrefix + "Error: Input parameter numDto returned an EMPTY number string!")
	}

	err = d2.SetNumStr(newNumStr)
