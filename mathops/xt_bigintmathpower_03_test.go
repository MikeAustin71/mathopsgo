...
func TestBigIntMathPower_MinimumRequiredPrecision_05(t *testing.T) {
  ...
  ePrefix := "TestBigIntMathPower_MinimumRequiredPrecision_05"
  ...
  _, err = new(BigIntMathPower).BigIntNumMinRequiredPrecision(base, exponent)

  if err == nil {
    t.Errorf("%v\n"+
      "Expected an Error, BUT NO ERROR WAS RETURNED!\n"+
      "Function Call:\n"+
      " _, err = new(BigIntMathPower).BigIntNumMinRequiredPrecision(\n"+
      "  base, exponent)\n"+
      "base= '%v'\n"+
      "exponent= '%v'\n"+
      "The maximum precision required execeeds the\n"+
      "maximum limit of a uint number.\n"+
      "Maximum Uint32 limit = '%v'\n\n",
      ePrefix,
      baseNumStr,
      exponentNumStr,
      math.MaxUint32)
...
func TestBigIntMathPower_MinimumRequiredPrecision_06(t *testing.T) {

  ePrefix := "TestBigIntMathPower_MinimumRequiredPrecision_06"

  baseInt := 312345

  basePrecisionUint := uint(5)

  exponentUint64 := uint64(12)

  //                                         1         2         3
  //                              0.1234567890123456789012345678901234567890
  //expectedResultNumStr := "862213.261066416994040251585138553420"

  expectedResult := uint(60)

  base, err := new(BigIntNum).NewInt(baseInt, basePrecisionUint)

  if err != nil {
    t.Errorf("%v\n"+
      "Error returned by:\n"+
      "base, err := new(BigIntNum).\n"+
      " NewInt(baseInt, basePrecisionUint)\n"+
      "baseInt= '%v'\n"+
      "basePrecisionUint= '%v'\n"+
      "Error= '%v'\n\n",
      ePrefix,
  ...
  exponent, err := new(BigIntNum).NewUint64(exponentUint64, 0)
  ...
  if err != nil {
    t.Errorf("%v\n"+
      ...
      "exponentUint64= '%v'"+
      "Error= '%v'\n\n",
      ePrefix, exponentUint64, err.Error())
    return
  }
